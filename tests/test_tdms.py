...
GOOD_TDMS_FILES = list(static_dir.glob("good/*.tdms"))
BAD_TDMS_FILES = list(static_dir.glob("bad/*.tdms"))
...
def test_content(filename):
    ...
    test_file_prop = {
        ...
        "good_tdms_2.tdms": {
            "no_channels": 24,
            "total_sum": 41137.14263
            + 227876.64,  # Sum of all values inside the channels
            ...
            + ["m/s"] * 3
            + ["hPa"]
            + ["%", "mm", "hits", "sec", "sec", "hits/H", "hits/H", "V"],
    ...
    assert len(signals) == test_file_prop[filename.name]["no_channels"]

    tot_sum = 0
    for signal, s_name, s_unit_str, s_len, s_fs, s_g in zip(  # type: ignore  # noqa: B905
        signals,
        test_file_prop[filename.name]["channel_names"],
        test_file_prop[filename.name]["unit_str"],
        test_file_prop[filename.name]["length"],
        test_file_prop[filename.name]["fs"],
        test_file_prop[filename.name]["group"],
    ):
    ...
    # Total sum of the data should match between the original file and
    # the imported
    assert tot_sum == pytest.approx(test_file_prop[filename.name]["total_sum"])
...
def test_compliance_sep005(filename):
    ...
    for signal in signals:
        Sep005Data.model_validate(signal)


@pytest.mark.parametrize("filename", BAD_TDMS_FILES)
def test_import_faulty_tdms(filename):
    """
    Test the import of a faulty tdms, should return a UserWarning and an
    empty signal list

    :return:
    """

    file_path = Path(static_dir) / "bad" / filename
    with pytest.warns(UserWarning, match=str(filename)):
        signals = read_tdms(file_path)
...
def test_write_tdms(tmp_path):
    """
    Test that checks to write a tdms file from a SEP005 compliant
    structure and then reads it again
    """
    signals = [
        {
            "name": "test_01",
    ...
    for signal_r, signal in zip(signals_read, signals):  # noqa: B905
